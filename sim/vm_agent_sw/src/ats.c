...
u8 m_axis_h2c_has_data() {
	...
	if(!(data & H2C_SHIM_FIFO_NOT_EMPTY_MASK))
		return 0;
	else
		return 1;
}
...
void pop_m_axis_h2c_fifo() {
	...
	VM_AGENT_SHIM(H2C_SHIM_FIFO_POP) = 0x00000001;
	VM_AGENT_SHIM(H2C_SHIM_FIFO_POP) = 0x00000000;
}
...
u8 read_at_from_m_axis_h2c(u32 *pa_hi, u32 *pa_lo) {
	u32 data = VM_AGENT_SHIM(VM_AGENT_SHIM_STATUS);
	...
	*pa_hi = VM_AGENT_SHIM(H2C_SHIM_PA_HI);
	*pa_lo = VM_AGENT_SHIM(H2C_SHIM_PA_LO);
	return 1;
}
...
int probe_and_read_m_axis_h2c(struct m_axis_h2c *in_struct) {
	...
	for(int i = 0; i < 10; i++) {
		if(i < 8) {
			in_struct->tdata[i] = *(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + i * 4);
		}
		else if(i == 9) {
			in_struct->tuser_crc = *(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x20);
		}
		else {
			in_struct->tuser_mdata = *(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x28);
		}
	}

	// Pop the FIFO and clearing the signal
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x18) = 0x00000001;
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x18) = 0x00000000;*/
...
void write_h2c_byp_in(struct h2c_byp_in *out_struct) {
	...
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR)    	= out_struct->raddr[0];
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x04) = out_struct->raddr[1];

	// Sending the write address
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x08) = out_struct->waddr[0];
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x0C) = out_struct->waddr[1];
	...
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x10) = dword;
	...
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x14) = dword;

	// Pushing the data out of the FIFO and clearing the signal
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x18) = 0x00000002;
	*(u32 *)(XPAR_VM_AGENT_QDMA_SHIM_0_S00_AXI_BASEADDR + 0x18) = 0x00000000;
